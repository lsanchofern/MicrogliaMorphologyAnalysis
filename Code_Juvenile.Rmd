...
data_nuc = pd.read_csv('./data/MyExpt_AllIba1_nuc.csv')
data_nuc=data_nuc[data_nuc.Children_Microglia_Count>0] #filter so you only have the ones with actual processes traced 
data_nuc=data_nuc.add_prefix('soma_')

data_microglia = pd.read_csv('./data/MyExpt_AllMicroglia.csv')

data= pd.concat([data_nuc.reset_index(drop=True),data_microglia.reset_index(drop=True)], axis=1)

#show top 5 rows to double-check everything looks normal 
data.head()
```
```{python}
#create mask for removing zeros (we don't want microglia with zeros branches)
data=data[data.soma_ObjectSkeleton_NumberBranchEnds_MorphologicalSkeleton>0]
data.head()
```
```{python}
...
my_palette=[(0.3,0.3,0.3), 'red', (0.7,0.7,0.7), (1.0,0.5,0.5)]
sns.violinplot(x="soma_Group",
            y="soma_AreaShape_Eccentricity",
            data=data,
            palette=my_palette,
            orient='v', 
            order=['TdT', 'Cyr61','TdT_MD','Cyr61_MD'],
            inner="box",
            inner_kws=dict(box_width=15, whis_width=2, color=".3")).set(xlabel="", ylabel="Eccentricity")
...
#cleaned_data=data.drop(['Slice'],axis=1)
#cleaned_data=data.drop(['Object_Number'],axis=1)
#cleaned_data.head()
```
```{python}
...
segmentation_std=scaler.fit_transform(data[features])
...
pca.explained_variance_ratio_ #how much of the variance is explained by each of the seven individual components 
...
plt.show()
```
Ok so from this we want 3 components because that explains 80% of the varaiblility

```{python}
#now we perform PCA with the chosen number of components (3)
pca=PCA(n_components=3)

#fit the model to our data with the selected number of components (3)
pca.fit(segmentation_std)
...
cleaned_cluster_averages=cluster_averages.drop(index=['Component 1', 'Component 2','Component 3', 'Segment K-means PCA'])
...
#tdt no MD
#add last column with pca k-means clustering labels 
df_scaled['Cluster']=df_segm_pca_kmeans['Segment']
...
for j in range(21): 
 # temp=pd.DataFrame(data.groupby(['soma_Group','soma_Metadata_Mouse'])[features[j]].median())
...
```
```{python}
temp=pd.DataFrame(data.groupby(['soma_Group','soma_Metadata_Mouse'])[features[2]].median())
temp
```
...
for j in range (21):   
  #kruskal wallis test 
  result, f=stats.kruskal(data[features[j]].loc[data['soma_Group']=='TdT'], data[features[j]].loc[data['soma_Group']=='TdT_MD'],
      data[features[j]].loc[data['soma_Group']=='Cyr61'],
      data[features[j]].loc[data['soma_Group']=='Cyr61_MD'])
  pval_featurename.append(features[j])
  pval_kruskal.append(f)    
  #dunn's posthoc tests 
  temp=[data[features[j]].loc[data['soma_Group']=='TdT'], data[features[j]].loc[data['soma_Group']=='TdT_MD'],
      data[features[j]].loc[data['soma_Group']=='Cyr61'],
      data[features[j]].loc[data['soma_Group']=='Cyr61_MD']]
  
  p= sp.posthoc_dunn(temp, p_adjust='holm')
  p.index.names=[features[j]]
  p.rename(index={1:'TdT',2:'TdT_MD',3:'Cyr61',4:'Cyr61_MD'},inplace=True)
  pval_dunn.append(p)
  print(p)
#make dataframe with Kruskal wallis results   
pvals_kruskal_cp=pd.DataFrame(pval_kruskal, index=pval_featurename)

df_pval_dunn=pd.concat(pval_dunn, 
  keys=map(lambda d: d.index.name, pval_dunn), axis=0)
df_pval_dunn.columns=['TdT','TdT_MD','Cyr61','Cyr61_MD']

pvals_kruskal_cp.to_csv('./PVals_Kruskal_cp.csv')
df_pval_dunn.to_csv('./Dunns_Kruskal_cp.csv')
...
df_percent['Segment']=df_percent['Segment'].replace({"first":"amoeboid","second":"hyper-ramified","third": "rod-like","fourth":"ramified"})
print(df_percent)
df_percent.to_csv('./juv_all_data.csv')
...
```
```{r}
all_data=read.csv("juv_all_data.csv")
print(all_data)
...
```
```{r}
# posthoc test looking at group 
library(emmeans)
sink(file="pairwise_tests_juvenile.txt")
...
all_data = pd.read_csv('./juv_all_data.csv')

plt.figure(figsize=(12,10))

sns.set_theme("talk")
sns.set_theme(style="ticks")
hue_order=['TdT','Cyr61','TdT_MD','Cyr61_MD']
ax1=sns.boxplot(x='Segment',y='percent',hue='Group',data=all_data,palette=my_palette,
            hue_order=hue_order, whis=[0,100],
...
```
```{python}
#plot but separate by viral group and manipulation 
sns.set()
sns.set_theme("talk")
sns.set_theme(style="ticks")
...
#WT 
TdT=all_data.loc[all_data['Group']=='TdT']
TdT_MD=all_data.loc[all_data['Group']=='TdT_MD']
Cyr61=all_data.loc[all_data['Group']=='Cyr61']
Cyr61_MD=all_data.loc[all_data['Group']=='Cyr61_MD']

hue_palette=sns.color_palette("rocket", 4)
sns.boxplot(data=TdT, x="Segment",y="percent",ax=axes[0,0],
            whis=[0,100],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],hue='Segment',palette=hue_palette)
sns.boxplot(data=Cyr61, x="Segment",y="percent",ax=axes[0,1],
            whis=[0,100],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],hue='Segment',palette=hue_palette)
sns.boxplot(data=TdT_MD, x="Segment",y="percent",ax=axes[1,0],
            whis=[0,100],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],hue='Segment',palette=hue_palette)
sns.boxplot(data=Cyr61_MD, x="Segment",y="percent",ax=axes[1,1],
            whis=[0,100],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],hue='Segment',palette=hue_palette)
            
sns.stripplot(data=TdT, x="Segment",y="percent",ax=axes[0,0],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],size=4, color=".3")
sns.stripplot(data=Cyr61, x="Segment",y="percent",ax=axes[0,1],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],size=4, color=".3")
sns.stripplot(data=TdT_MD, x="Segment",y="percent",ax=axes[1,0],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],size=4, color=".3")
sns.stripplot(data=Cyr61_MD, x="Segment",y="percent",ax=axes[1,1],
            order=['amoeboid', 'rod-like','ramified','hyper-ramified'],size=4, color=".3")
sns.despine()            
axes[0,0].set_title('tdT no MD')
axes[0,1].set_title('CCN1 no MD')
axes[1,0].set_title('tdT 5D MD')
axes[1,1].set_title('CCN1 5D MD')
plt.rcParams['pdf.use14corefonts'] = True
